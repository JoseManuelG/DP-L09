...
public interface MessageRepository extends JpaRepository<Message, Integer> {
	...
	//Dashboard - The minimum, the average, and the maximum number of messages sent per actor. Part1
	@Query("select count(m) from Message m where m.isSender=true and m.sender is not null")
	Double avgMessagesSentPerActor();

	//Dashboard - The minimum, the average, and the maximum number of messages sent per actor. Part2
	@Query("select count(m) from Message m where m.isSender=true and m.sender is not null")
	...
	//Dashboard - The minimum, the average, and the maximum number of messages received per actor. Part1
	@Query("select count(m) from Message m where m.isSender=false and m.recipient is not null")
	Double avgMessagesReceivedPerActor();

	//Dashboard - The minimum, the average, and the maximum number of messages received per actor. Part2
